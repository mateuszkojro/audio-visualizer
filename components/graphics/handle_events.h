...
void HandleMousePress(std::array<Button, BUTTONS_COUNT> &butt_vec,
  ...
  switch (i) {
	case NUMBER_OF_SAMPLES_UP:fourier_data->number_of_samples += 1;
	...
	case NUMBER_OF_SAMPLES_UP_DOWN:
	  ...
	  break;
	case SCALING_FACTOR_UP:fourier_data->scaling_factor *= 0.99;

	  break;
	case SCALING_FACTOR_DOWN:fourier_data->scaling_factor *= 1.01;
	...
	case WINDING_START_UP:
	  ...
	  break;
	case WINDING_START_DOWN:fourier_data->winding_start -= 1;

	  break;
	case WINDING_END_UP:fourier_data->winding_end += 1;

	  break;
	case WINDING_END_DOWN:fourier_data->winding_end -= 1;

	  break;
	case WINDING_STEP_UP:fourier_data->winding_step += 1;
	...
	case SPEED_UP:
	  ...
	  break;
	case SLOW_DOWN:fourier_data->sleep_for += std::chrono::milliseconds(10);
	  break;
	...
	case CROSSHAIR:butt_vec[CROSSHAIR].Press();
	  break;

	case SNAP_FUNCTION:butt_vec[SNAP_FUNCTION].Press();
	  break;

	case NORMALIZE_FUNCTION:butt_vec[NORMALIZE_FUNCTION].Press();
	  break;

	case REFLECT_FUNCTION:butt_vec[REFLECT_FUNCTION].Press();
	  break;
...
void HandleMouseScrollUp(std::array<Button, BUTTONS_COUNT> &butt_vec,
  ...
  switch (i) {
	...
	case NUMBER_OF_SAMPLES_UP_DOWN:fourier_data->number_of_samples += 1;

	  break;
	case SCALING_FACTOR_UP:
	case SCALING_FACTOR_DOWN:fourier_data->scaling_factor *= 0.99;
	...
	case WINDING_STEP_DOWN:fourier_data->winding_step += 1;
	  break;
