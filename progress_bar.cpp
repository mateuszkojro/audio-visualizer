...
int main(int argc, char *argv[]) {
    ...
    while (bar_head_position != WINDOW_WIDTH) {
        ...
        for (int i = 0; i < bar_head_position; i++) {
            int j = 0;
            for (; j < line_thickness; j++) {
                if (i % 50 == 0) toggle = !toggle;

                if (toggle)surface->draw_point({i, WINDOW_HEIGHT - j - bar_height}, 5, {0, 210, 0});
                else surface->draw_point({i, WINDOW_HEIGHT - j - bar_height}, 5, {0, 210, 0, 3});
            }
        }
