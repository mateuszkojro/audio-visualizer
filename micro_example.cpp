...
int main(int argc, char *argv[]) {
    ...
    std::string path = "../files\\example.wav";
    if (path.empty()) {
        exit(1);
    }
    ...
    desiredPlaybackSpec.callback = audio_callback;

    /// Start reading the audio file
    if (SDL_OpenAudioDevice( NULL, SDL_FALSE, &desiredPlaybackSpec, &gReceivedPlaybackSpec, SDL_AUDIO_ALLOW_FORMAT_CHANGE ) < 0) {
